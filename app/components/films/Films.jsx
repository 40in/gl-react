...
export default class Film extends Component {
    ...
    componentDidMount(){
        ...
        if (this.props.filmUrl) {
            fetch(this.props.filmUrl)
                .then(result => result.json())
                .then(filmItems => this.setState({filmItems}));
        }
    }
    ...
    render(){
        return <div>{this.state.filmItems.title ? <div>Title: {this.state.filmItems.title}</div> : ""}</div>
    }
}
